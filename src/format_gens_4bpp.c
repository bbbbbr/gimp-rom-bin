...
#define GENS_BITS_PER_PIXEL_4BPP            4    // 4 bits per pixel mode
...
static int bin_decode_image_data_gens_4bpp(void * file_data, long int * file_size, long int *file_offset, int width, int height, unsigned char * image_data)
{
    ...
    if (*file_size < ((width / (8 / GENS_BITS_PER_PIXEL_4BPP)) * height))
        return -1;
...
static int bin_encode_image_data_gens_4bpp(unsigned char * ptr_source_image_data, int source_width, int source_height, long int * ptr_output_size, unsigned char * ptr_output_data)
{
    unsigned char * ptr_image_pixel;
    ...
    if (*ptr_output_size < (source_width * source_height) / (8 / GENS_BITS_PER_PIXEL_4BPP))
        return -1;
...
static int bin_load_color_data_gens_4bpp(unsigned char * ptr_color_map_data, int color_map_size)
{
    int status = 0;
...
int bin_decode_to_indexed_gens_4bpp(void * ptr_file_data, long int file_size, int * ptr_width, int * ptr_height, unsigned char ** ptr_ptr_image_data, unsigned char ** ptr_ptr_color_map_data, int * color_map_size,  int image_mode)
{
    ...
    *ptr_ptr_image_data = malloc(*ptr_width * *ptr_height);
    ...
    // Read the image data
    if (0 != bin_decode_image_data_gens_4bpp(ptr_file_data,
                                            &file_size,
                                            &file_offset,
                                            *ptr_width,
                                            *ptr_height,
    ...
    *ptr_ptr_color_map_data = malloc(*color_map_size * DECODED_COLOR_MAP_BYTES_PER_PIXEL);
    ...
    if (0 != bin_load_color_data_gens_4bpp(*ptr_ptr_color_map_data, *color_map_size))
        return -1;
...
int bin_encode_to_indexed_gens_4bpp(unsigned char * ptr_source_image_data, int source_width, int source_height, long int * ptr_output_size, unsigned char ** ptr_ptr_output_data, int image_mode)
{
    // Set output file size based on Width, Height and bit packing
    *ptr_output_size = (source_width * source_height) / (8 / GENS_BITS_PER_PIXEL_4BPP);
    ...
    *ptr_ptr_output_data = malloc(*ptr_output_size);
    ...
    if (0 != bin_encode_image_data_gens_4bpp(ptr_source_image_data,
                                            source_width,
                                            source_height,
                                            ptr_output_size,
                                           *ptr_ptr_output_data));
